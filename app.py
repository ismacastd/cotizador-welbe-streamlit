...
def main():
    ...
    st.caption("DEBUG BUILD: catch-all activo")
    ...
    if st.button("CALCULAR", type="primary"):
        ...
        with st.expander("Debug: inputs al core"):
            st.write("geo_col usado:", geo_col)
            st.write("municipios_comp:", municipios_comp)
            st.write("estudios:", list(sel_est))
            st.write("margen:", margen)
...
# ✅ CATCH-ALL GLOBAL (esto evita redacción sí o sí)
try:
    main()
except Exception as e:
    st.error("ERROR REAL (catch-all):")
    st.write(str(e))
